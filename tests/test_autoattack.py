...
class AutoAttackTests(unittest.TestCase):
    ...
    def test_har_import_and_web_payload(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("Skill Stats", aa.render_console(tmp))
            self.assertIn("findings", aa.api_payload(tmp, "status"))
            self.assertIn("skill_runs", aa.api_payload(tmp, "skill_stats"))
    ...
    def test_skill_manifest_normalize_validate_and_load(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            rc, schema = self._capture_json(["skills", "schema"])
            self.assertEqual(rc, 0)
            self.assertIn("name", schema["required"])
            self.assertEqual(schema["properties"]["schema_version"]["enum"], [aa.SKILL_SCHEMA_VERSION])
