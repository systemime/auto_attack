...
7. **解释/评估**：`skills list --summary` 展示分页与分布，`skills explain` 展示候选、计划、跳过原因、冲突和分数，`skills eval` 用 JSON cases 检查期望/拒绝的 plans/candidates，run 记录 `skill_routing_summary` 事件。
...
| Observability/tracing/eval | events/tool_runs/skill_runs/report + `skills explain` + `skills eval` JSON cases + `skill_routing_summary` events | 部分达标；无 trace UI |
